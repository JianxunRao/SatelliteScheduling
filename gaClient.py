from platypus import Problem, Integer, GeneticAlgorithm
import json
import requests
...
objective_weights['FitAW'] = 1
objective_weights['FitCS'] = 1
objective_weights['FitTR'] = 1
objective_weights['FitGU'] = 1
...
def evaluate(x):
    ...
    objective = objective_weights['FitAW']*result['FitAW'] + \
           objective_weights['FitCS']*result['FitCS'] + \
           objective_weights['FitTR']*result['FitTR'] + \
           objective_weights['FitGU']*result['FitGU']

    tracer_logger.log({'variable': request_json['schedule'], 'objective': {'weighted_objective': objective}})
